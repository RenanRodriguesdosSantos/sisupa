...
export default class Outros extends Component{
    constructor(props){
        ...
        axios.get(this.api + "exames/dados/outros/" + this.state.consulta)
        .then(response => {
    ...
    handleChange(e){
        ...
        exames[e.target.id.substring(6,e.target.id.length)].observacao = e.target.value;
    ...
    salvar(e){
        ...
        axios.post(this.api + "exames/outros/atendimento/store",{exames: exames})
        .then(e => {
            salvo();
                        ...
                        <div>
                            <div className="table-responsive">
                                <table className="table table-striped">
                                    <thead>
                                        <tr scope="row">
                                            <th scope="col" colSpan="3"> Descrição do Exame</th>
                                    ...
                                    <tbody>
                                        {
                                            this.state.exames.map(
                                                ...
                                                <tr key={row.id}>
                                                    ...
                                                    <td colSpan="2"><input type="text" id={"outros" + row.index} className="form-control" placeholder="Observação" onChange={this.handleChange} value={this.state.exames[row.index].observacao}/></td>
                                                </tr>
                                            )
                                        }
                                    </tbody>
