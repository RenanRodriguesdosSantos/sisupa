...
export default class Imagem extends Component{
    constructor(props){
        ...
        axios.get(this.api + "exames/dados/imagem/" + this.state.consulta)
        .then(response => {
    ...
    salvar(e){
        ...
        axios.post(this.api + "exames/imagem/atendimento/store",{exames: exames})
        .then(e => {
            salvo();
                                    ...
                                    <tbody>
                                        {
                                            this.state.exames.map(
                                                ...
                                                <tr key={row.id}>
                                                    ...
                                                    <td colSpan="2"><input type="text" id={"imagem" + row.index} className="form-control" placeholder="Observação" onChange={this.handleChange} value={this.state.exames[row.index].observacao}/></td>
                                                </tr>
                                            )
                                        }
                                    </tbody>
