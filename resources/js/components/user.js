...
export default class User extends Component{
    constructor(){
        super();
        this.state = {user: [], senhaAntiga: "", novaSenha: "", confirmar: "", userId: "", userName: ""};
        axios.get("/user")
        .then(response => {this.setState({user: response.data, userId: response.data.id, userName: response.data.conselho? response.data.name + "  CRM: " + response.data.conselho: response.data.name})});
    ...
    render(){
        return(
            <div className="col-md-2">
                ...
                <input type="text" id="userName" onChange={this.handleChange} value={this.state.userName} className="d-none"/>
